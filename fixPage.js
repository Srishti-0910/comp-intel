...
export default function Home() {
  ...
  useEffect(() => {
    fetchSalaries()
      .then((data) => {
        setAllSalaries(data.salaries || []);
        setLoading(false);
      })
      .catch(() => setLoading(false));
  }, []);

  const filtered = allSalaries.filter((s: any) => {
    const matchSearch =
  ...
  const avgTotal = allSalaries.length ? Math.round(allSalaries.reduce((a: number, b: any) => a + b.totalComp, 0) / allSalaries.length) : 0;
  const maxTotal = allSalaries.length ? Math.max(...allSalaries.map((s: any) => s.totalComp)) : 0;
  const topCompany = allSalaries.find((s: any) => s.totalComp === maxTotal) as any;
    ...
    <main className="min-h-screen" style={{ background: "linear-gradient(135deg, #f0f4ff 0%, #faf5ff 50%, #f0fffe 100%)" }}>
      ...
      <div className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #4f46e5 0%, #7c3aed 40%, #0891b2 100%)" }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center text-white">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {[
              { label: "Salary Entries", value: allSalaries.length, icon: "📋", color: "#818cf8" },
              { label: "Top Companies", value: new Set(allSalaries.map((s: any) => s.company)).size, icon: "🏢", color: "#34d399" },
              { label: "Roles Tracked", value: new Set(allSalaries.map((s: any) => s.role)).size, icon: "👨‍💻", color: "#f472b6" },
              { label: "Cities Covered", value: new Set(allSalaries.map((s: any) => s.location)).size, icon: "📍", color: "#fbbf24" },
            ].map((stat) => (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            { label: "Avg Total Comp", value: avgTotal ? "₹" + (avgTotal).toFixed(1) + "L" : "...", icon: "💰", gradient: "linear-gradient(135deg, #6366f1, #8b5cf6)", sub: "Across all companies" },
            { label: "Highest Package", value: maxTotal ? "₹" + (maxTotal).toFixed(1) + "L" : "...", icon: "🏆", gradient: "linear-gradient(135deg, #f59e0b, #ef4444)", sub: topCompany ? "At " + topCompany.company : "" },
            { label: "Cities Available", value: new Set(allSalaries.map((s: any) => s.location)).size + " Cities", icon: "🌍", gradient: "linear-gradient(135deg, #10b981, #0891b2)", sub: "Pan India coverage" },
          ].map((card) => (
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 font-medium">Loading salary data...</p>
          </div>
        ) : (
          <>
            <div className="mb-6">
            ...
            <div className="flex items-center justify-between mb-4 bg-white rounded-2xl px-5 py-3 shadow-sm border border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm text-gray-500">
                  Showing <span className="font-black text-indigo-600 text-base">{filtered.length}</span> of <span className="font-bold text-gray-800">{allSalaries.length}</span> entries
                </p>
              </div>
              {filtered.length !== allSalaries.length && (
                <button
