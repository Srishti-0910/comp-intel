...
import { Salary } from "@/data/salaries";
import { formatCurrency, getCompanySlug, getLevelColor } from "@/lib/utils";
...
type SortKey = "total" | "base" | "bonus" | "stock" | "experience";
        ...
        <thead>
          <tr className="bg-gradient-to-r from-indigo-50 to-blue-50 border-b border-gray-200">
            ...
            {headers.map((h) => (
              <th
                key={h.key}
                className="text-left px-5 py-4 font-semibold cursor-pointer hover:text-indigo-600 transition-colors"
                onClick={() => handleSort(h.key)}
              >
                <div className="flex items-center gap-1.5">
                  {h.label}
                  <ArrowUpDown size={13} className={clsx(
                    "transition-colors",
                    sortKey === h.key ? "text-indigo-600" : "text-gray-300"
                  )} />
                </div>
              </th>
            ))}
        ...
        <tbody className="divide-y divide-gray-50">
          {sorted.map((s, index) => (
            <tr key={s.id} className="hover:bg-indigo-50/40 transition-colors group">
              ...
              {/* Rank */}
              <td className="px-5 py-4 text-gray-300 font-medium text-xs">
                {index + 1}
              </td>
              ...
              <td className="px-5 py-4">
                <Link
                  href={`/company/${getCompanySlug(s.company)}`}
                  className="flex items-center gap-2"
                >
                  <span className={clsx(
                    "px-2.5 py-1 rounded-lg text-xs font-bold",
                    COMPANY_COLORS[s.company] || "bg-gray-100 text-gray-700"
                  )}>
                    {s.company}
                  </span>
                </Link>
